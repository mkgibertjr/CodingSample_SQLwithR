...
```{r Exercise2_datadownload_RUN, eval=FALSE,message = FALSE}
...
```{r Exercise2_datadownload_run, echo = FALSE}
...
```{r Exercise3_Problem5_run, include=FALSE}
...
```{r Exercise3_Problem8_last}

sqlQuery(conn,
    "SELECT DISTINCT YEAR FROM CROP_DATA ORDER BY YEAR DESC LIMIT 1")
...
sqlQuery(conn,"SELECT * FROM CROP_DATA WHERE YEAR >= '2000-12-31' ORDER BY AVG_YIELD DESC LIMIT 10")

```

```{r Exercise3_Problem11_summed,eval=FALSE}

sqlQuery(conn,
    "SELECT CROP_TYPE,GEO,SUM(AVG_YIELD) 
     FROM CROP_DATA 
...
```{r Exercise3_Problem11_summed_run,echo=FALSE}

sqlQuery(conn,"SELECT CROP_TYPE,GEO,SUM(AVG_YIELD) FROM CROP_DATA WHERE YEAR >= '2000-12-31' GROUP BY CROP_TYPE,GEO ORDER BY 3 DESC LIMIT 10")
...
sqlQuery(conn,
    "SELECT YEAR,GEO,CROP_TYPE,PRODUCTION 
     FROM CROP_DATA 
...
sqlQuery(conn,"SELECT YEAR,GEO,CROP_TYPE,PRODUCTION FROM CROP_DATA WHERE GEO='Canada' and CROP_TYPE='Wheat' ORDER BY YEAR DESC LIMIT 1")
